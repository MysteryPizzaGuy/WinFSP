    ...
    PVOID Buffer, ULONG BufferLength, PULONG PBytesTransferred)
{
    ...
    if (FspFileSystemAcquireDirectoryBuffer(&FileContext->DirBuffer, 0 == Marker, &DirBufferResult))
    {
		...
		//TestFolder
		memset(DirInfo, 0, sizeof *DirInfo);
		Length = (ULONG)wcslen(L"TestFolder");
		DirInfo->Size = (UINT16)(FIELD_OFFSET(FSP_FSCTL_DIR_INFO, FileNameBuf) + Length * sizeof(WCHAR));
		...
		DirInfo->FileInfo.IndexNumber = 0;
		DirInfo->FileInfo.HardLinks = 0;
		memcpy(DirInfo->FileNameBuf, L"TestFolder", Length * sizeof(WCHAR));
		FspFileSystemFillDirectoryBuffer(&FileContext->DirBuffer, DirInfo, &DirBufferResult);
		//
		//TestFile
		memset(DirInfo, 0, sizeof *DirInfo);
		Length = (ULONG)wcslen(L"TestFile");
		DirInfo->Size = (UINT16)(FIELD_OFFSET(FSP_FSCTL_DIR_INFO, FileNameBuf) + Length * sizeof(WCHAR));
		...
		DirInfo->FileInfo.IndexNumber = 0;
		DirInfo->FileInfo.HardLinks = 0;
		memcpy(DirInfo->FileNameBuf, L"TestFile", Length * sizeof(WCHAR));
		FspFileSystemFillDirectoryBuffer(&FileContext->DirBuffer, DirInfo, &DirBufferResult);
		//
		//TestFile2
		memset(DirInfo, 0, sizeof *DirInfo);
		Length = (ULONG)wcslen(L"TestFile2");
		DirInfo->Size = (UINT16)(FIELD_OFFSET(FSP_FSCTL_DIR_INFO, FileNameBuf) + Length * sizeof(WCHAR));
		DirInfo->FileInfo.FileAttributes = FILE_ATTRIBUTE_NORMAL;
		DirInfo->FileInfo.ReparseTag = 0;
		DirInfo->FileInfo.FileSize =
		...
		DirInfo->FileInfo.IndexNumber = 0;
		DirInfo->FileInfo.HardLinks = 0;
		memcpy(DirInfo->FileNameBuf, L"TestFile2", Length * sizeof(WCHAR));
		FspFileSystemFillDirectoryBuffer(&FileContext->DirBuffer, DirInfo, &DirBufferResult);
		//
        FspFileSystemReleaseDirectoryBuffer(&FileContext->DirBuffer);
